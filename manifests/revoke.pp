...
define openvpn::revoke (
  ...
  $revocation_command = $openvpn::easyrsa_version ? {
    ...
    '3.0' => ". ./vars && yes yes | ./easyrsa revoke ${name}; echo \"exit $?\" | grep -qE '(error 23|exit (0|2))' && touch revoked/${name}",
  }

  $renew_command = $openvpn::easyrsa_version ? {
    '2.0'   => ". ./vars && KEY_CN='' KEY_OU='' KEY_NAME='' KEY_ALTNAMES='' openssl ca -gencrl -out ${openvpn::server_directory}/${server}/easy-rsa/keys/crl.pem -config ${openvpn::server_directory}/${server}/easy-rsa/openssl.cnf",
    '3.0'   => ". ./vars && EASYRSA_REQ_CN='' EASYRSA_REQ_OU='' openssl ca -gencrl -out ${server_directory}/${server}/easy-rsa/keys/crl.pem -config ${server_directory}/${server}/easy-rsa/openssl.cnf",
  ...
  exec { "revoke certificate for ${name} in context of ${server}":
    ...
    provider => 'shell',
    notify   => Exec["renew crl.pem on ${server}"],
  }

  exec { "renew crl.pem on ${server}":
